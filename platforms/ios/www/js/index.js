...
initiateMain: function(references){
    ...
    + '<a id="otherButton" class="button" href="#" onClick="checkVersionAndDownload();">?</a>'
    + '</div>';
    $("body").append(toolbar);
    ...
    var languageList = '<ul id="languages" title="Medical expressions in..." selected="true">';
    for (var i in dict) {
        languageList = languageList + '<li><a href="javascript:;" onClick="getWords(\''+i+'\');">' + i +'</a></li>';
    }
...
function getWordDetails(elementId){
    ...
    + '<audio controls id="audio" style="position: absolute; bottom: 0; width:100%;">'
    + '<source src="'+nativePath.replace('BabelApp/','BabelApp') + dict[lastLanguage][elementId].file_path+'" type="audio/mpeg">'
    + '</audio>';
...
gotFile: function(fileEntry){
    ...
    console.log(dict);
    localVersion = jsonArr[0]["version"];
...
downloadFile: function(){
    ...
    window.requestFileSystem(
                             LocalFileSystem.PERSISTENT,
                             0,
                             packageDownloader.onRequestFileSystemSuccess,
                             packageDownloader.fail
                             );
},
...
onRequestFileSystemSuccess: function(fileSystem){
    ...
    fileSystem.root.getFile(
                            'dummy.html',
                            {create: true, exclusive: false},
                            packageDownloader.onGetFileSuccess,
                            packageDownloader.fail
                            );
},
...
onGetFileSuccess: function(fileEntry){
    ...
    fileTransfer.download(
                          'http://alazgulec.net/pack.zip',
                          path + 'pack.zip',
                          function(file) {
                          zip.unzip(path + 'pack.zip', path, packageDownloader.status);
                          console.log('download complete: ' + file.toURL());
                          packageDownloader.showLink(file.toURL());
                          },
                          function(error) {
                          console.log('download error source ' + error.source);
                          console.log('download error target ' + error.target);
...
executeJSON: function(fileURL){
    var jsonArr = JSON.parse(packageDownloader.loadJSON(fileURL));
    console.log(jsonArr);
...
gotFile: function(fileEntry){
    ...
    var localUrl = fileEntry.toURL();
    var nativeURL = fileEntry.toNativeURL();
    nativePath = nativeURL.replace("references.json", "");
    console.log('Loaded local path: ' + localPath);
    console.log('Loaded local url: ' + localUrl);
    console.log('Native URL: ' + nativeURL);
    var jsonArrString = getLocalReferences.loadJSON(localUrl);
...
function checkVersion(){
    var jsonString = jQuery.ajax({
                       url : 'http://alazgulec.net/references/references.json',
                       async : false,
                       dataType : 'json'
                       }).responseText;
    ...
    console.log(jsonArr);
    var upToDateVersion = jsonArr[0]["version"];

    if(upToDateVersion>localVersion){
        return false;
    }
    else{
        return true;
